...
# ==============================================================================
# 1. FUNZIONI DI SUPPORTO (Modularizzazione)
# ==============================================================================

def setup_function_spaces(domain, gdim):
    """Definisce gli elementi finiti e gli spazi funzionali."""
    ...
    Ve_p = element("DG", cellname, 0)
    W = fem.functionspace(domain, mixed_element([Ve_u, Ve_m, Ve_p]))
    return W

def apply_boundary_conditions(domain, W, facet_tag, L2):
    """Applica le condizioni di Dirichlet (Meccaniche e Magnetiche)."""
    ...
    if bottom_facets_inc.size > 0:
        # bottom_dofs_ux = fem.locate_dofs_topological(W.sub(0).sub(0), fdim, bottom_facets_inc)
        bottom_dofs_uy = fem.locate_dofs_topological(W.sub(0).sub(1), fdim, bottom_facets_inc)
        # bc_ux_bottom = fem.dirichletbc(zero, bottom_dofs_ux, W.sub(0).sub(0))
        bc_uy_bottom = fem.dirichletbc(zero, bottom_dofs_uy, W.sub(0).sub(1))
        # bc_list.append(bc_ux_bottom)
...
def run_solver(F_tot, J_tot, U, bcs_mixed, domain):
    """Configura e lancia il solutore non lineare (Newton-Raphson + MUMPS)."""
...
def postprocess_and_visualize(domain, cell_tag, u, phi, p, bl, hl, b_func, h_func, 
                              sigma_solid, J_det_expr, chi, h_field, Q0, geom, gdim, L1, L2):
    """Tutto il codice di visualizzazione PyVista, Matplotlib e salvataggio VTK."""
    
    # Recupero le variabili geometriche per i grafici 1D
    cx, cy = geom.center
...
# ==============================================================================
# 2. FUNZIONE PRINCIPALE DEL MODELLO
# ==============================================================================

def solve_model(h_factor=1.0, run_postprocessing=True):
    """
    Costruisce e risolve il modello. Ritorna il dominio e gli oggetti
    della soluzione per permettere test di convergenza.
    """
    geom = Geometry()
    L1 = geom.L1
    L2 = geom.L2
    ...
    # NB: Usa il parametro h_factor se lo hai aggiunto in build_mesh per fare il test di convergenza,
    # altrimenti lascia la chiamata base come nel tuo snippet originale.
    domain, cell_tag, facet_tag = build_mesh(MPI.COMM_WORLD, geom, h_factor=h_factor)
    ...
    # Spazi funzionali
    W = setup_function_spaces(domain, gdim)
    ...
    u_, phi_, p_ = ufl.split(U)
    w_, v_, q_ = ufl.split(Z)
    du_, dphi_, dp_ = ufl.split(dU)
    U.name = "U"
    ...
    # energies
    W_solid_mech = 0.5 * G * (I1 - 2) 
    Omega_solid_mech = W_solid_mech + p_ * (J - 1)
    W_vac_mech = 0.5 * mu_v * (I1 - 2 - 2*ufl.ln(J)) + 0.5 * K_v * (J - 1)**2

    Omega = Omega_solid_mech - 0.5 * mu0 * mu * ufl.dot(ufl.inv(F.T)*hl, ufl.inv(F.T)*hl)
    ...
    F_p = q_ * (J - 1) * dx(1) + q_ * p_ * dx(2)
    F_tot = F_u + F_phi + F_p
    J_tot = ufl.derivative(F_tot, U, dU)

    # Risoluzione
    bcs_mixed = apply_boundary_conditions(domain, W, facet_tag, L2)
    run_solver(F_tot, J_tot, U, bcs_mixed, domain)
    ...
    # Se richiesto, lancia tutto il blocco di visualizzazione e plot 1D
    if run_postprocessing:
        postprocess_and_visualize(domain, cell_tag, u, phi, p, bl, hl, b_func, h_func, 
                                  sigma_solid, J_det_expr, chi, h_field, Q0, geom, gdim, L1, L2)
        
    return domain, W, U

# ==============================================================================
# ENTRY POINT (Avvio automatico se lanci lo script normalmente)
# ==============================================================================
if __name__ == "__main__":
    solve_model(run_postprocessing=True)
